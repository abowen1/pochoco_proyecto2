...
module pochoco_periph (
  ...
  // Synchronize external buttons; release debouncing is handled in assembly.
  reg [3:0] btn_meta_q, btn_sync_q;
  always @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin btn_meta_q <= 0; btn_sync_q <= 0; end
    else begin btn_meta_q <= btn_i; btn_sync_q <= btn_meta_q; end
  end

  // Free-running 32-bit clock counter; wraps modulo 2^32.
  reg [31:0] cycle_q;
  reg [31:0] led_start_q;
  always @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      cycle_q <= 32'b0;
      led_start_q <= 32'b0;
      ...
      // Same edge as the LED update: software can measure from that event.
      if (access & we_i & (off == 6'd1)) led_start_q <= cycle_q;
    end
  end
  ...
  always @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) rdata_o <= 32'b0;
      ...
      case (off)
        6'd2: rdata_o <= {28'b0, btn_sync_q}; // Buttons
        6'd3: rdata_o <= cycle_q;     // 0x0c: current cycle count
        6'd4: rdata_o <= led_start_q; // 0x10: last LED write timestamp
